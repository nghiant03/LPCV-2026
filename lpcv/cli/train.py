...
Each model type has its own sub-command (``videomae``, ``r2plus1d``) that
accepts model-specific options.  All commands share the same data-loading
...
def _run_training(
    ...
    model's default presets, constructs the trainer config, and runs
    training.  Saves ``val_transform.json`` alongside the model for
    ...
    train_transform = build_transform(spec.train_preset)
    val_transform = build_transform(spec.val_preset)
    ...
    if num_gpus > 1:
        ...
        elastic_launch(launch_config, _launch)(config, train_ds, eval_ds, spec.val_preset)
    else:
        _launch(config, train_ds, eval_ds, spec.val_preset)
